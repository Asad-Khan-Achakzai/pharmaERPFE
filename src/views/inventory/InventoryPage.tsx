...
const InventoryPage = () => {
  ...
  /** Factory/catalog cost, average landed cost, and cost-valued totals — same gate as other company financials */
  const canSeeInventoryAdminCosts = hasPermission('admin.access')
  ...
  const detailColumns = useMemo<ColumnDef<InventoryRow, any>[]>(() => {
    const cols: ColumnDef<InventoryRow, any>[] = [
      ...
      detailHelper.accessor((r) => Number(r.productId?.mrp ?? 0), {
        id: 'mrp',
        header: 'MRP',
        cell: ({ getValue }) => formatPKR(getValue())
      }),
      detailHelper.accessor((r) => Number(r.productId?.tp ?? 0), {
        id: 'tp',
        header: 'TP',
        cell: ({ getValue }) => formatPKR(getValue())
      })
    ]
    if (canSeeInventoryAdminCosts) {
      cols.push(
    ...
    cols.push(
      detailHelper.display({
        ...
        header: 'Actions',
        cell: ({ row }) => (
          <IconButton size='small' onClick={() => setViewDetail(row.original)}>
  ...
  const summaryColumns = useMemo<ColumnDef<SummaryRow, any>[]>(() => {
    const cols: ColumnDef<SummaryRow, any>[] = [
      ...
      summaryHelper.accessor('mrp', {
        header: 'MRP',
        cell: ({ getValue }) => formatPKR(Number(getValue() ?? 0))
      }),
      summaryHelper.accessor('tp', {
        header: 'TP',
        cell: ({ getValue }) => formatPKR(Number(getValue() ?? 0))
      })
    ]
    ...
    cols.push(
      ...
      summaryHelper.display({
        ...
        header: 'Actions',
        cell: ({ row }) => (
          <IconButton size='small' onClick={() => setViewSummary(row.original)}>
            <i className='tabler-eye text-textSecondary' />
          </IconButton>
        )
      })
  ...
  const kpiData: { title: string; value: string; icon: string; color: string; tooltip?: string }[] = useMemo(
    () => [
      ...
      canSeeInventoryAdminCosts
        ? {
            ...
            value: formatPKR(totals.totalValue),
            icon: 'tabler-coin',
            color: 'success',
            tooltip: TOOLTIP_INVENTORY_VALUE_KPI
          }
        : {
            title: 'Inventory at TP',
            value: formatPKR(summaryTotalTp),
            icon: 'tabler-currency-dollar',
            color: 'success',
            tooltip: TOOLTIP_TRADE_PRICE_NO_SHIPPING
          }
    ],
    [totals.uniqueProducts, totals.totalUnits, totals.totalValue, summaryTotalTp, canSeeInventoryAdminCosts]
  )
  // TODO: Low stock KPI
                  ...
                  <div className='flex flex-col items-end gap-1 p-4 border-bs'>
                    ...
                    <Typography variant='caption' color='text.secondary' className='max-w-[min(100%,42rem)] text-end'>
                      {canSeeInventoryAdminCosts
                        ...
                        : 'TP is quantity × product TP (no shipping in TP).'}
                    </Typography>
                  </div>
                )}
              </TabPanel>
                  ...
                  <div className='flex flex-col items-end gap-1 p-4 border-bs'>
                    ...
                    <Typography variant='caption' color='text.secondary' className='max-w-[min(100%,42rem)] text-end'>
                      {canSeeInventoryAdminCosts
                        ...
                        : 'TP is sum of (qty × TP) per product—shipping is not in TP.'}
                    </Typography>
                  </div>
                )}
              </TabPanel>
          ...
          {viewDetail && (() => {
            ...
            <Grid container spacing={3} className='pbs-4'>
              ...
              <Grid size={{ xs: 12 }}>
                ...
                <Typography variant='caption' color='text.secondary' display='block' className='mbs-1'>
                  Quantity × TP per unit. Shipping is <strong>not</strong> included in TP.
                  {canSeeInventoryAdminCosts
                    ? ' Shipping is reflected in average cost when stock is received on a GRN or transfer.'
                    : ''}
                </Typography>
              </Grid>
            </Grid>
          ...
          {viewSummary && (() => {
            ...
            <Grid container spacing={3} className='pbs-4'>
              ...
              <Grid size={{ xs: 12 }}>
                ...
                <Typography variant='caption' color='text.secondary' display='block' className='mbs-1'>
                  Total quantity × TP per unit. <strong>Shipping is not</strong> part of this figure.
                  {canSeeInventoryAdminCosts
                    ? ' Cost (above) is where transfer shipping is spread across products when stock came in on a transfer.'
                    : ''}
                </Typography>
              </Grid>
            </Grid>
